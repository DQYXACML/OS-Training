...
{
    uint8_t code[446];                          // 引导代码区，前446字节
    part_item_t part_item[MBR_PRIMARY_PART_NR]; // 分区表，4个表项
    uint8_t boot_sig[2];                        // 引导标志 AA55
} mbr_t;
...
typedef struct _partinfo_t
{
    ...
    struct _disk_t *disk;      // 所属的磁盘，反向引用

    // https://www.win.tue.nl/~aeb/partitions/partition_types-1.html 分区表链接
    enum
    {
